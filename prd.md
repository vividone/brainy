...
### 8.6.1 Coverage, measured — and the source we now work from

**Revised: coverage was assumed, then counted.** `npm run coverage` prints
every skill by curriculum, subject, class and strand. Run for the first time in
August 2026, it showed the pack was badly lopsided:

| B1 | B2 | B3 | B4 | B5 | B6 |
|---|---|---|---|---|---|
| 21 | 32 | **51** | 30 | 23 → | 13 → |

Basic 3 — the class the first child was in — had fifty-one skills. Basic 6 had
thirteen, of which **one** was maths. A product sold as "Basic 1 to 6" cannot be
nearly empty for the two classes that sit common entrance. Filling that is the
current content priority, ahead of new subjects.

**Primary source: the Lagos State Unified Scheme of Work for Primary Schools,
2021 Edition** — the state's own curriculum document, following NERDC, published
free as PDFs (Mathematics, English Studies, BST, National Values, History, for
Primary 1–3 and 4–6). It gives topic, term and week for every class, which is
exactly what §8.7 lists as the highest-value item to gather. The scans have no
text layer; the page images extract cleanly and can be read directly.

Two things it settled:

- **Quantitative Reasoning is not a separate syllabus.** The scheme embeds a QR
  sample in *every* maths topic — factor-split boxes, machine diagrams,
  percentage-to-fraction matching. Keeping QR a separate *subject* is still
  right for reporting (§1), but its *content* should track the maths topics term
  by term, and currently does not.
- **Only the topic list and its sequence are taken.** Those are curriculum
  facts. Every question is generated by us. The aggregator hosting the PDFs
  states "the entirety of this site is protected by copyright, all rights
  reserved", so its lesson notes and exam questions are not ours to use, and
  none of them are in the product.

**Reading the questions is a separate check from running them.** `npm run smoke`
proves a generator produces well-formed, answerable items; it cannot see that
`2/4` offered beside a correct answer of `1/2` is a second right answer, or that
`2⁴` and `4²` are the same number. `npm run sample` prints real generated
questions with their options and answers, and that is what caught both. Every
new skill goes through both.
...
- **The first 20 families get a free year, covering every child on their tablet.** They are the people taking a risk on an unproven app, and their feedback is worth more than their subscription.
- **One licence covers a family, not a child.** Multi-child support shipped in phase 6: several children share one device, each with their own progress, character, pet and report, and a licence opens every subject for all of them. This is deliberate rather than unenforced — `subjectOpen()` in `src/state/entitlement.ts` never looks at which child is playing. Per-seat pricing would mean a younger sibling locked out of a subject their brother is using, which is the least defensible moment a family could meet. The `children` column on `subscriptions` survives as a record of family size and grants nothing.
